 ...
 const query = 'SELECT mail, @sender := sender_addr, reception_addr, (SELECT sum(b.amount*b.price_dollar) FROM dxd_transactions t INNER JOIN dxd_blockchain b ON b.tx_hash = t.tx_hash WHERE t.sender_addr = @sender AND b.tx_date >= 1620234000) AS total_amount_usd, (SELECT count(*) FROM dxd_transactions t INNER JOIN dxd_blockchain b ON b.tx_hash = t.tx_hash WHERE t.sender_addr = @sender AND b.tx_date >= 1620234000) AS nb_of_txs, (SELECT min(b.tx_date) FROM dxd_transactions t INNER JOIN dxd_blockchain b ON b.tx_hash = t.tx_hash WHERE t.sender_addr = @sender AND b.tx_date >= 1620234000) AS first_tx FROM dxd_smartlinkcopy ORDER BY first_tx';
...
////////////////////// DB ////////////////////////////////
 ...
 async function main() {
     ...
     await writeToCSV(data, "scenario12export3")
 }
